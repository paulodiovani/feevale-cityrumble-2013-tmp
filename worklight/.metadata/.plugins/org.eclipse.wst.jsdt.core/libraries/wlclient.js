...
__WLClient = function() {
     ...
     * @param policy The policy object has the following properties. Both of these properties are optional.<ul>
     * 				<li><b>eventStorageEnabled</b> A Boolean value that determines where events are stored. If the value is <code>true</code>, events may be stored in HTML5 session storage. If the value is <code>false</code>, events that are waiting for transmission are stored in memory. The default value is <code>false</code>.
     * 				<br> The value <code>true</code> should be used only if the developer of the policy is not concerned about privacy issues involved in storing the events, which include the user's device context.</li>
     *				<li><b>interval</b> The transmission interval, in milliseconds. The default value is 60000 (one minute). Before events are transmitted, they are accumulated in storage.</li></ul> 	
     */
     ...
     * 
     * @param event Mandatory. The event object that is being transmitted. The event object is either a literate object, or a reference to an object.
     * @param immediate Optional. A Boolean flag that indicates whether the transmission should be immediate (<code>true</code>), or should be based on the transmission policy's interval (<code>false</code>). 
